    ...
    O: OltpQueryRunner,
{
    pub fn new(
        ...
        Self { eav_reader, eav_writer, sqs_bus, oltp_executor }
    }
    ...
    async fn fetch_pending_events(&self, tenant_id: &str) -> Result<Vec<Value>, DomainError> {
        ...
        let pending_rows = self.oltp_executor.run_oltp_query(tenant_id, &query_pending).await?;
        ...
        if let Ok(failed_rows) = self.oltp_executor.run_oltp_query(tenant_id, &query_failed).await {
            if let Some(arr) = failed_rows.as_array() {
                for item in arr {
                    let retry_at = item.get("retry_at").and_then(|v| v.as_i64()).unwrap_or(0);
                    let retry_count = item.get("retry_count").and_then(|v| v.as_i64()).unwrap_or(0);
    ...
    async fn try_claim_event(&self, tenant_id: &str, event_id: &str) -> Result<Option<Value>, DomainError> {
        ...
        let status = event.get("status").and_then(|v| match v {
        ...
        attrs.insert("status".to_string(), DatomValue::Str("PROCESSING".to_string()));
        ...
        if !event_hydrated.is_empty() {
            ...
            event_hydrated.insert("outbox_id".to_string(), DatomValue::Str(event_id.to_string()));
        }
    ...
    async fn mark_delivered(&self, tenant_id: &str, event_id: &str) -> Result<(), DomainError> {
        let mut attrs = HashMap::new();
        attrs.insert("status".to_string(), DatomValue::Str("DELIVERED".to_string()));
    ...
    async fn mark_failed(&self, tenant_id: &str, event_id: &str, max_retries: i64) -> Result<(), DomainError> {
        ...
        let event = self.eav_reader.pull(tenant_id, event_id, None).await?;
        let current_retries = event.get("retry_count").and_then(|v| match v {
    ...
    pub async fn reset_orphaned_processing(&self, tenant_id: &str, ttl_ms: i64) -> Result<usize, DomainError> {
        let now = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_millis() as i64;
        ...
        for event in pending_events {
            ...
            match self.try_claim_event(tenant_id, event_id).await {
                Ok(Some(claimed_event)) => {
                    ...
                    let detail_type = claimed_event.get("detail_type").and_then(|v| v.as_str()).unwrap_or("unknown");
