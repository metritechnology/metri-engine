...
fn resolve_trigger(entity: &str, payload: &Value) -> Result<Trigger, DomainError> {
    let s = |k: &str| payload.get(k).and_then(|v| v.as_str()).filter(|v| !v.is_empty());

    if let Some(meter) = payload.get("meter_based_trigger").filter(|v| !v.is_null()) {
        return Ok(Trigger::Telemetry { expr: telemetry_expr(meter)? });
    }
    if let Some(cron) = s("cron_expression") {
        return Ok(Trigger::Cron { expr: cron.to_string() });
    }
    if let Some(iso) = s("reminder_datetime") {
        return Ok(Trigger::ExactTime { epoch_secs: iso_to_epoch_secs(iso)? });
    }
...
fn telemetry_expr(meter: &Value) -> Result<String, DomainError> {
    let metric = meter.get("metric_code")
        .or_else(|| meter.get("meterId"))
        .and_then(|v| v.as_str())
        .ok_or_else(|| DomainError::janus(
            ErrorCode::Jns001,
            "[SagaBuilder] meter_based_trigger sin metric_code".to_string(),
        ))?;
    let op = meter.get("operator").and_then(|v| v.as_str()).unwrap_or("GT");
    let sym = match op {
        "GT" | ">"  => ">",
        "LT" | "<"  => "<",
        "GTE" | ">=" => ">=",
        "LTE" | "<=" => "<=",
        "EQ" | "==" => "==",
        other => return Err(DomainError::janus(
            ErrorCode::Jns001,
            format!("[SagaBuilder] operador telemétrico no soportado: '{other}'"),
        )),
    };
    let threshold = meter.get("threshold").or_else(|| meter.get("value"))
        .and_then(|v| v.as_f64())
        .ok_or_else(|| DomainError::janus(
            ErrorCode::Jns001,
...
fn epoch_secs_of(v: &Value) -> Option<i64> {
    let n = v.as_i64().or_else(|| v.as_str().and_then(|s| s.parse::<i64>().ok()))?;
...
fn prenotify_offsets(payload: &Value) -> Vec<i64> {
    for key in ["prenotify_before_minutes", "prenotify_minutes_array"] {
        if let Some(arr) = payload.get(key).and_then(|v| v.as_array()) {
            let mut v: Vec<i64> = arr.iter().filter_map(|x| x.as_i64()).filter(|n| *n > 0).collect();
            v.sort_unstable();
            v.dedup();
            return v;
        }
...
async fn resolve_source(
    ...
    match src.len() {
        ...
        _ => {
            ...
            let entity = reader.pull(tenant_id, ref_id, Some(&[target_attr])).await.ok()?;
...
pub async fn build_saga_projections(
    ...
    let Some(mapping) = model.shadow_sagas_mapping.as_ref().and_then(|m| m.as_object()) else {
        return Ok(Vec::new());
    };
    if mapping.is_empty() {
        return Ok(Vec::new());
    ...
    let job_model = crate::codice::global().get_model("scheduled_job").ok_or_else(|| {
        DomainError::janus(
            ErrorCode::Jns001,
            "[SagaBuilder] 'scheduled_job' no está en el Códice — no se puede proyectar".to_string(),
        )
    })?;

    let trigger = resolve_trigger(&model.entity, payload)?;
    let tz = payload.get("iana_timezone").and_then(|v| v.as_str()).unwrap_or("UTC");
    ...
    for offset_min in offsets {
        ...
        obj.insert("idempotency_hash".into(), json!(sha256_hex(&[
            parent_id, &expr, &offset_min.to_string()
        ])));
        ...
        for (dest, src) in mapping {
            let Some(src_arr) = src.as_array() else { continue };
        ...
        let attrs = validator::validate_payload(job_model, &job_payload, tenant_id, true)
            .map_err(|e| DomainError::janus(
                ErrorCode::Jns001,
                format!("[SagaBuilder] la proyección de '{}' no valida contra scheduled_job: {e:?}",
                        model.entity),
            ))?;
