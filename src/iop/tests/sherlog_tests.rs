...
fn test_fault_severity_parsing() {
    ...
    assert_eq!(
        FaultSeverity::from_catalog_str("error"),
        FaultSeverity::Error
    );
    assert_eq!(
        FaultSeverity::from_catalog_str("fatal"),
        FaultSeverity::Fatal
    );
    assert_eq!(
        FaultSeverity::from_catalog_str("anything_else"),
...
async fn test_process_fault_success() {
    let notifier = MockFaultNotifier {
        notifications: Mutex::new(vec![]),
    };
    let olap = MockWriteChannel {
