...
fn create_temp_models_dir() -> (std::path::PathBuf, impl FnOnce()) {
    ...
    let path = std::env::temp_dir().join(format!("metri_test_models_{}_{}", std::process::id(), count));
...
fn test_registry_compilation() {
    ...
    fs::write(dir.join("test_note.json"), serde_json::to_string(&model_json).unwrap()).unwrap();
...
fn test_duplicate_entity_prevention() {
    ...
    fs::write(dir.join("m1.json"), serde_json::to_string(&model_1).unwrap()).unwrap();
    fs::write(dir.join("m2.json"), serde_json::to_string(&model_2).unwrap()).unwrap();
    
    let res = CodeRegistry::build(&dir);
    assert!(res.is_err(), "Expected compilation to fail due to duplicate entity");
...
fn test_invalid_scope_provider() {
    ...
    let res = CodeRegistry::build(&dir);
    assert!(res.is_err(), "Expected compilation to fail due to invalid sequence scope provider");
    let err = res.err().unwrap();
...
fn test_dashboard_bi_entity_registered() {
    ...
    assert!(res.is_ok(), "Expected main registry compilation to succeed, got {:?}", res.err());
    let (registry, _rules) = res.unwrap();
    
    let model = registry.get_model("dashboardBI");
    assert!(model.is_some(), "dashboardBI entity should be registered in the CodeRegistry");
    let model = model.unwrap();
    assert_eq!(model.entity, "dashboardBI");
    
    assert!(model.attributes.iter().any(|a| a.name == "name" && a.attr_type == AttrType::String));
    assert!(model.attributes.iter().any(|a| a.name == "description" && a.attr_type == AttrType::String));
    assert!(model.attributes.iter().any(|a| a.name == "widgets" && a.attr_type == AttrType::Json));
    assert!(model.attributes.iter().any(|a| a.name == "created_at" && a.attr_type == AttrType::Epoch));
    assert!(model.attributes.iter().any(|a| a.name == "updated_at" && a.attr_type == AttrType::Epoch));
}
