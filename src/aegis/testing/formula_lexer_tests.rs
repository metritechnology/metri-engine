...
fn test_tokenize_unexpected_token() {
    ...
    assert!(matches!(err, FormulaError::UnexpectedToken { position: 2, .. }));
}

#[test]
fn test_tokenize_formula_too_long() {
    let long_formula = "a".repeat(4097);
    assert!(matches!(tokenize(&long_formula).unwrap_err(), FormulaError::FormulaTooLong { .. }));
}

#[test]
fn test_tokenize_too_many_tokens() {
    let tokens_formula = "1 + ".repeat(257); // 514 tokens
    assert!(matches!(tokenize(&tokens_formula).unwrap_err(), FormulaError::TooManyTokens { .. }));
}
