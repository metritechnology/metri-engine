...
use crate::aegis::sql::cte_compiler::{
    build_comparison_cte_query, resolve_ts_col, time_frame_to_cond, TableExpression,
};
use crate::aegis::sql::dialect::{AthenaDialect, SqlDialect};
...
fn is_structural_field(f: &str, config: &HybridEntityConfig) -> bool {
    match f {
        "id" | "tenant_id" | "tenant/id" | "_tenant" | "created_at" | "updated_at"
        | "createdAt" | "updatedAt" | "timestamp" | "ingested_at" | "metadata" => return true,
        _ => {}
    }
...
pub fn compile_hybrid_meter_reading_sql(
    ...
    let config = get_hybrid_config("meter_reading").ok_or_else(|| {
        DomainError::aegis(
            ErrorCode::Aeg001,
            "Missing meter_reading hybrid config".to_string(),
        )
    })?;
...
pub fn compile_sql_with_dialect(
    ...
    let has_cte_comps = if let Some(comps) = &ast.comparisons {
        comps.iter().any(|c| {
            matches!(
                c.comp_type.as_str(),
                "TIME_SHIFT_RELATIVE" | "TIME_SHIFT_SHORTCUT" | "TIME_SHIFT_ABSOLUTE" | "SMART"
            )
        })
    } else {
        false
