...
pub fn dim_to_bucket_expr(d: &Dimension, dialect: &dyn SqlDialect) -> String {
    let attr_name = d.attribute.as_deref()
        .or(d.field.as_deref())
        .unwrap_or("event_ts");
    ...
    match ast.output_cast {
        OutputCast::TIMESERIES => {
            if let Some(dims) = &ast.group_by {
                for d in dims {
                    ...
                    let alias = d.attribute.as_deref()
                        .or(d.field.as_deref())
                        .unwrap_or("bucket")
                        .to_string();
        ...
        OutputCast::KPI | OutputCast::PIE | OutputCast::BUBBLE => {
            ...
            if let Some(sem_ms) = &ast.semantic_measures {
                for sm in sem_ms {
                    select_exprs.push((Expr::cust(&format!("NULL /* semantic:{} */", sm.metric_key)), Some(sm.metric_key.clone())));
                }
            }
        }
        OutputCast::TABLE => {
            let mut has_sel = false;
            let has_dims = ast.group_by.as_ref().map(|a| !a.is_empty()).unwrap_or(false);
