...
pub fn compile_metric(
    ...
    let sec_attr = m.secondary_attribute.as_deref().or(m.secondary_field.as_deref());
    ...
    let field = if is_hybrid && is_count {
        ...
        attr.map(|a| format!("\"{}\"", col_id_str(a))).unwrap_or_else(|| "*".to_string())
    };
    ...
    let eff_field = if let Some(filter_val) = &m.filter {
        if !filter_val.is_null() {
            ...
            let then_val = if fn_str == "COUNT" { "1".to_string() } else { field.clone() };
    ...
    let mut agg_call = match fn_str.as_str() {
        ...
        "CORRELATION" => {
            let sec = sec_attr.map(|a| format!("\"{}\"", col_id_str(a))).unwrap_or_else(|| eff_field.clone());
    ...
    let null_safe_fns = vec![
        "SUM", "AVG", "MIN", "MAX", "MEDIAN", "STD_DEV", "VARIANCE",
