...
use crate::temporal::comparison::{
    ...
    AnalyticalComparison, ComparisonType, ShiftShortcut,
};
use sea_query::{Alias, CommonTableExpression, Cond, Expr, Query, SimpleExpr, SelectStatement};
...
pub fn time_frame_to_cond(start_ts: Option<i64>, end_ts: Option<i64>, ts_col: &str, dialect: &dyn SqlDialect) -> Option<sea_query::Condition> {
...
pub fn to_analytical_comparison(c: &ComparisonDef) -> Option<AnalyticalComparison> {
    ...
    Some(AnalyticalComparison {
        comp_type,
        relative_granularity: c.relative_granularity.clone().unwrap_or_else(|| "day".to_string()),
...
pub fn build_comparison_cte_query(
    ...
    if ast.output_cast == OutputCast::TIMESERIES {
        if let Some(dims) = &ast.group_by {
            for d in dims {
                ...
                let alias = d.attribute.as_deref().or(d.field.as_deref()).unwrap_or("bucket").to_string();
        ...
        for (i, comp_val) in comps.iter().enumerate() {
            if let Some(comp) = to_analytical_comparison(comp_val) {
                match comp.comp_type {
                    ComparisonType::TimeShiftRelative | ComparisonType::TimeShiftShortcut | ComparisonType::TimeShiftAbsolute => {
                        let tz = "UTC"; // Can be dynamic if needed
                        if let Some(period) = resolve_comparison_period(&comp, time_range, tz) {
                            let prev_clause = time_frame_to_cond(Some(period.prev_start), Some(period.prev_end), &ts_col, dialect);
                    ...
                    ComparisonType::Smart => {
                        ...
                        let hist_clause = time_frame_to_cond(hist_range.start_ts, hist_range.end_ts, &ts_col, dialect);
                            ...
                            if let Some(metrics) = &ast.metrics {
                                if let Some(m) = metrics.iter().find(|m| {
                                    ...
                                    let computed_alias = if let Some(n) = name {
                                        ...
                                        let suffix = attr.map(col_id_str).unwrap_or_else(|| "total".to_string());
                                        format!("{}_{}", agg.to_lowercase(), suffix)
                                    };
                                    computed_alias == *alias
                                }) {
                                    raw_field = m.attribute.as_deref().or(m.field.as_deref()).map(col_id_str).unwrap_or_else(|| "*".to_string());
                                }
                            }
                            
                            let field_for_stats = if raw_field == "*" { "1.0".to_string() } else { format!("\"{}\"", raw_field) };
                            stat_exprs.push((Expr::cust(&format!("AVG({})", field_for_stats)), Some(format!("mean_{}", alias))));
                            stat_exprs.push((Expr::cust(&dialect.format_stddev_samp(&field_for_stats)), Some(format!("std_{}", alias))));
                        }
    ...
    if ast.output_cast == OutputCast::TIMESERIES {
        if metric_exprs.is_empty() {
            ...
            final_select.expr_as(Expr::cust(&format!("c.\"{}\"", bucket_alias)), Alias::new(&bucket_alias));
        }
        ...
        for (cte_name, _) in &time_shift_ctes {
            for (_, alias) in &metric_exprs {
                final_select.expr_as(Expr::cust(&format!("{}.\"{}\"", cte_name, alias)), Alias::new(&format!("{}_{}", cte_name, alias)));
            }
        }
        ...
        for (_, alias) in &metric_exprs {
            final_select.expr_as(Expr::cust(&format!("c.\"{}\"", alias)), Alias::new(&format!("current_{}", alias)));
        }
        ...
        for (smart_name, _) in &smart_ctes {
            for (_, alias) in &metric_exprs {
                let z_expr = format!(
                    "(c.\"{}\" - {}.\"{}\") / NULLIF({}.\"{}\", 0)",
                    alias,
                    smart_name, format!("mean_{}", alias),
                    smart_name, format!("std_{}", alias)
                );
                final_select.expr_as(Expr::cust(&z_expr), Alias::new(&format!("z_score_{}", alias)));
            }
        }
    }
    ...
    if ast.output_cast == OutputCast::TIMESERIES {
        for (cte_name, _) in &time_shift_ctes {
            final_select.left_join(
                Alias::new(cte_name),
                Expr::cust(&format!("c.\"{}\" = {}.\"{}\"", bucket_alias, cte_name, bucket_alias))
            );
        }
    } else {
        for (cte_name, _) in &time_shift_ctes {
    ...
    with_query.cte(CommonTableExpression::new()
    ...
    let sql = with_query.query(final_select).to_string(sea_query::PostgresQueryBuilder);
