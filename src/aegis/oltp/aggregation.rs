...
fn get_strategy(agg_fn: AggregationFunction) -> Box<dyn AggregationStrategy> {
    match agg_fn {
        ...
        _ => {
            warn!(
                "[Aegis Agg] AggregationFunction desconocida {:?} — SUM fallback",
                agg_fn
            );
            Box::new(SumStrategy)
        }
...
pub fn apply_metrics_fbs(
    ...
    for metric in metrics {
        ...
        let alias = metric.name.as_deref().map(String::from).unwrap_or_else(|| {
        ...
        let eff_rows: Vec<&Value> = if let Some(filter_node) = &metric.filter {
            rows.iter()
                .filter(|r| eval_filter_node(r, filter_node))
                .collect()
        } else {
            rows.iter().collect()
        };
        ...
        let sec_vals: Vec<f64> = if sec_key.is_empty() {
            ...
            eff_rows
                .iter()
                .filter_map(|r| r.get(sec_key).and_then(|v| v.as_f64()))
                .collect()
        };
...
pub fn derive_columns(
    ...
    all_keys.sort_by_key(|k| {
        if dim_attrs.contains(k) {
            0u8
        } else if metric_aliases.contains(k) {
            2u8
        } else {
            1u8
        }
    ...
    all_keys
        .iter()
        .map(|k| {
            let sample_val = rows.iter().find_map(|r| r.get(k).filter(|v| !v.is_null()));
...
fn infer_col_type(key: &str, sample: Option<&Value>) -> &'static str {
    ...
    if matches!(
        key,
        "created_at" | "updated_at" | "deleted_at" | "ingested_at" | "timestamp"
    ) {
        return "timestamp";
    }
    match sample {
        Some(Value::Number(n)) => {
            if n.as_f64().map(|v| v > 1e11).unwrap_or(false) {
                "timestamp"
            } else {
                "number"
            }
