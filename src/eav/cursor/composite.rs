...
impl CompositeCursor {
    ...
    pub fn encode(&self) -> Result<String, DomainError> {
        let bytes = rmp_serde::to_vec(self)
            .map_err(|e| DomainError::eav(ErrorCode::Eav003, format!("cursor encode error: {e}")))?;
    ...
    pub fn decode(token: &str, current_fingerprint: u64) -> Result<Self, DomainError> {
        let bytes = URL_SAFE_NO_PAD
            .decode(token)
            .map_err(|_| DomainError::eav(ErrorCode::Eav003, "cursor base64 inválido".to_string()))?;

        let cursor: CompositeCursor = rmp_serde::from_slice(&bytes)
            .map_err(|e| DomainError::eav(ErrorCode::Eav003, format!("cursor decode error: {e}")))?;

        if cursor.query_fingerprint != current_fingerprint {
            return Err(DomainError::eav(ErrorCode::Eav003, "cursor stale: el query cambió entre páginas".to_string()));
        }
