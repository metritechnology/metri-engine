...
pub fn compress_readings(readings: &[IoTReading]) -> Vec<u8> {
    ...
    let csv: String = readings.iter()
        .map(|r| format!("{},{},{},{}", r.entity_id, r.attr_name, r.value, r.timestamp_ms))
        .collect::<Vec<_>>()
        .join("\n");
...
pub fn decompress_readings(blob: &[u8]) -> Vec<IoTReading> {
    let Ok(decompressed) = decompress_size_prepended(blob) else { return vec![] };
    let Ok(csv) = std::str::from_utf8(&decompressed) else { return vec![] };
...
mod tests {
    ...
    fn compress_decompress_roundtrip() {
        let readings = vec![
            IoTReading { entity_id: "e1".to_string(), attr_name: "temp".to_string(), value: 23.5, timestamp_ms: 1000 },
            IoTReading { entity_id: "e1".to_string(), attr_name: "temp".to_string(), value: 24.0, timestamp_ms: 2000 },
        ];
