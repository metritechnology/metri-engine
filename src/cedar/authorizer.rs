...
use cedar_policy::{Authorizer, Context, Entities, PolicySet, Request, Decision, Schema, EntityUid};
use std::str::FromStr;
...
pub static INVALIDATION_TX: once_cell::sync::Lazy<tokio::sync::broadcast::Sender<InvalidationMsg>> = once_cell::sync::Lazy::new(|| {
    ...
    pub fn insert(&self, token: &str, session: Session) {
        if let Ok(mut lock) = self.sessions.write() {
            if lock.len() >= MAX_SESSION_CACHE_SIZE && !lock.contains_key(token) {
                let to_remove: Vec<String> = lock.keys().take(MAX_SESSION_CACHE_SIZE / 5).cloned().collect();
                for k in to_remove {
                    lock.remove(&k);
                }
            }
                ...
                if let Ok(mut principal_lock) = cache_clone.write() {
                    match msg.entity_type.as_str() {
                        "user" => {
                            ...
                            tracing::info!("[CacheInvalidation] Evicted User {} from Principal Cache", msg.entity_id);
                        }
                        "role" => {
    ...
    async fn store_principal(&self, user_id: &str, principal: PrincipalData) -> Result<(), DomainError> {
        if let Ok(mut lock) = self.cache.write() {
            if lock.len() >= MAX_PRINCIPAL_CACHE_SIZE && !lock.contains_key(user_id) {
                let to_remove: Vec<String> = lock.keys().take(MAX_PRINCIPAL_CACHE_SIZE / 5).cloned().collect();
                for k in to_remove {
                    lock.remove(&k);
                }
            }
        ...
        if Self::is_master_only_entity(entity_type) {
            ...
            if !is_master && !is_system_bff {
                let msg = match action {
                    "mutate" => format!("Auth403: Only master tenant users can mutate {}", entity_type),
                    "read" => "Auth403: Only master tenant users can read tenants or quotas".to_string(),
                    "explore" => format!("Auth403: Only master tenant users can explore {}", entity_type),
                    _ => format!("Auth403: Only master tenant users can access {}", entity_type),
                };
...
pub async fn step1_extract_token<T>(
    ...
    let token = if let Some(auth_header) = req.metadata().get("authorization").and_then(|v| v.to_str().ok()) {
        ...
        return Err(DomainError::new(ErrorCode::Auth401, "Missing authorization or sid header").with_stage("cedar"));
    };
    ...
    let session = valkey_store.get_session(&token).await?
        .ok_or_else(|| DomainError::new(ErrorCode::Auth401, "Invalid or expired token").with_stage("cedar"))?;

    if session.user_id.is_empty() || session.tenant_id.is_empty() {
        return Err(DomainError::new(ErrorCode::Auth401, "Malformed session payload from Valkey").with_stage("cedar"));
    }
...
pub async fn step2_query_oltp(
    ...
    if let Some(cached) = cache.lookup_principal(user_id).await {
        if cached.status == "SUSPENDED" {
            return Err(DomainError::new(ErrorCode::Auth403, format!("User {user_id} is suspended")).with_stage("cedar"));
        }
        return Ok(cached);
    }

    let user_map = eav_reader.pull(tenant_id, user_id, None).await?;
    tracing::info!("[step2_query_oltp] tenant_id={}, user_id={}, user_map={:?}", tenant_id, user_id, user_map);
    if user_map.is_empty() {
        return Err(DomainError::new(ErrorCode::Auth403, format!("User {user_id} not found")).with_stage("cedar"));
    }

    let status = user_map.get("status").or_else(|| user_map.get("user/status"))
        .and_then(|v| match v {
...
pub async fn step3_consolidate(
    ...
    if user_data.roles.is_empty() {
        return Err(DomainError::new(ErrorCode::Auth403, "User has no roles assigned").with_stage("cedar"));
    }
    ...
    let group_locs_handle = tokio::spawn(async move {
        expand_hierarchy(&eav_reader_g1, &tenant_g1, "location", group_locations_roots).await
    });
    ...
    for boundary in &user_data.roles_boundaries {
        ...
        let final_permitted_locations = if !user_data.group_allowed_locations.is_empty() {
            if expanded_locations.is_empty() {
                ...
                expanded_locations.intersection(&expanded_group_locations).cloned().collect()
            }
        } else {
        ...
        let final_permitted_assets = if !user_data.group_allowed_assets.is_empty() {
            if expanded_assets.is_empty() {
                ...
                expanded_assets.intersection(&expanded_group_assets).cloned().collect()
            }
        } else {
...
pub fn step3b_validate_time_window(
    ...
    if !matches_any {
        return Err(DomainError::new(ErrorCode::Auth403, "Access outside allowed time window").with_stage("cedar"));
    }
        ...
        if is_mutational_action(action) {
            ...
            if let Some(domains) = resource.get("domains").and_then(|v| v.as_array()) {
                for d_val in domains {
                    ...
                    domain_dict.insert(domain.to_string(), serde_json::json!([{
                        "query_scope": "ALL",
...
fn collect_user_grants(principal: &PrincipalData) -> HashSet<String> {
    ...
    let all_domains: Vec<String> = if let Some(registry) = crate::codice::registry::global_opt() {
        ...
        vec![
            "project".to_string(), "work_order".to_string(), "asset".to_string(),
            "location".to_string(), "user".to_string(), "user_group".to_string(),
            "role".to_string(), "tenant".to_string(), "company".to_string(),
            "api_key".to_string(), "audit_log".to_string(), "form_template".to_string(),
            "provider".to_string(), "webhook_endpoint".to_string(), "dashboardBI".to_string(),
            "domain_plugin".to_string(), "domain_quota".to_string(), "tenant_plugin".to_string()
        ]
    };
    ...
    for boundary in &principal.roles_boundaries {
        for grant in &boundary.grants {
            if let Some(domain_val) = grant.get("domain").and_then(|v| v.as_str()) {
                ...
                let actions = if let Some(actions_arr) = grant.get("actions").and_then(|v| v.as_array()) {
...
fn step4_mutational(
    ...
    let principal_uid = format!("Metri::User::\"{}\"", principal.user_id).parse::<EntityUid>()
        .map_err(|e| DomainError::new(ErrorCode::Auth401, format!("Invalid principal UID: {e}")).with_stage("cedar"))?;
    let action_uid = format!("Metri::Action::\"{}\"", action).parse::<EntityUid>()
        .map_err(|e| DomainError::new(ErrorCode::Auth401, format!("Invalid action UID: {e}")).with_stage("cedar"))?;
    
    let resource_id = resource.get("entity_id").and_then(|v| v.as_str()).unwrap_or("");
    let resource_uid = format!("Metri::MetriResource::\"{}\"", resource_id).parse::<EntityUid>()
        .map_err(|e| DomainError::new(ErrorCode::Auth401, format!("Invalid resource UID: {e}")).with_stage("cedar"))?;

    let schema = get_cedar_schema();
    
    let user_grants = collect_user_grants(principal);
    let user_grants_json = serde_json::Value::Array(
        user_grants.into_iter().map(serde_json::Value::String).collect()
    );

    let principal_roles_json = serde_json::Value::Array(
        principal.roles.iter().map(|r| serde_json::Value::String(r.clone())).collect()
    );

    let entity_type = resource.get("entity_type").and_then(|v| v.as_str()).unwrap_or("project");
    ...
    let entities = Entities::from_json_str(&entities_json.to_string(), Some(schema))
        .map_err(|e| DomainError::new(ErrorCode::Auth403, format!("Failed to create Cedar entities: {e}")).with_stage("cedar"))?;

    if principal.roles_boundaries.is_empty() {
        if let Some(policy_set) = policy_cache.get("admin").or_else(|| policy_cache.values().next()) {
        ...
        for boundary in &principal.roles_boundaries {
            let policy_set = policy_cache.get(&boundary.role_id)
                ...
                .ok_or_else(|| DomainError::new(ErrorCode::Auth403, "PolicySet not compiled for role").with_stage("cedar"))?;
...
fn grant_allows_action(grant: &serde_json::Value, action: &str) -> bool {
    let check = |act: &str| {
        act == "*" || act == action || (action == "BulkIngestData" && (act == "CREATE" || act == "UPDATE"))
    };
...
fn step4_analytical(
    ...
    if let Some(domains) = resource.get("domains").and_then(|v| v.as_array()) {
        for d_val in domains {
            ...
            if (domain == "tenant" || domain == "domain_quota" || domain == "quota") && principal.tenant_id != master_tenant_id {
            ...
            if boundaries_json.is_empty() {
                return Err(DomainError::new(
                    ErrorCode::Auth403,
                    format!("Cedar DENY: No role authorized for action {action} on domain {domain}")
                ).with_stage("cedar"));
            }
        ...
        if let Ok(entity_map) = eav_reader.pull(&session.tenant_id, resource_id, None).await {
            if let Some(obj) = resource.as_object_mut() {
                for key in &["assigned_company_id", "company_id", "assigned_company", "company"] {
                    let entity_type = obj.get("entity_type").and_then(|v| v.as_str()).unwrap_or("");
...
fn extract_cedar_action<T>(req: &tonic::Request<T>) -> Result<String, DomainError> {
    if let Some(act) = req.metadata().get("x-metri-action").and_then(|v| v.to_str().ok()) {
        Ok(act.to_string())
    } else {
...
fn extract_cedar_resource<T>(req: &tonic::Request<T>) -> Result<serde_json::Value, DomainError> {
    let entity_type = req.metadata().get("x-metri-entity-type")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("project");

    let domains = req.metadata().get("x-metri-domains")
        .and_then(|v| v.to_str().ok())
        .map(|s| s.split(',').map(|d| d.trim().to_string()).collect::<Vec<String>>())
        .unwrap_or_else(|| vec![entity_type.to_string()]);

    let entity_id = req.metadata().get("x-metri-entity-id")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("");
...
async fn assemble_principal_graph(
    ...
    let user_type = match user_map.get("user_type").or_else(|| user_map.get("user/user_type")) {
        Some(DatomValue::Str(s)) => s.clone(),
        _ => "INTERNAL".to_string(),
    };
    
    let company_id = match user_map.get("company_id").or_else(|| user_map.get("user/company_id")) {
        Some(DatomValue::Str(s)) => s.clone(),
        _ => "".to_string(),
    };

    let status = match user_map.get("status").or_else(|| user_map.get("user/status")) {
        Some(DatomValue::Str(s)) => s.clone(),
        _ => "ACTIVE".to_string(),
    };
    ...
    if let Some(val) = user_map.get("role_ids").or_else(|| user_map.get("user/role_ids")) {
        ...
        for r_id in role_ids {
            ...
            let grants = match role_map.get("grants").or_else(|| role_map.get("role/grants")) {
                Some(DatomValue::Str(s)) => {
                    let parsed: serde_json::Value = serde_json::from_str(s).unwrap_or_else(|_| serde_json::json!([]));
                    if let serde_json::Value::Array(arr) = parsed {
                        let v: Vec<serde_json::Value> = arr.into_iter()
                            .map(|item| match item {
                                serde_json::Value::String(str_val) => {
                                    serde_json::from_str(&str_val).unwrap_or_else(|_| serde_json::json!(str_val))
                                }
                                _ => item,
                            })
                            .collect();
                ...
                Some(DatomValue::Array(arr)) => {
                    let v: Vec<serde_json::Value> = arr.iter()
            ...
            let permitted_locations = match role_map.get("permitted_locations").or_else(|| role_map.get("role/permitted_locations")) {
                Some(DatomValue::Array(arr)) => arr.clone(),
                Some(DatomValue::Str(s)) => vec![s.clone()],
                _ => vec![],
            };

            let permitted_assets = match role_map.get("permitted_assets").or_else(|| role_map.get("role/permitted_assets")) {
                Some(DatomValue::Array(arr)) => arr.clone(),
                Some(DatomValue::Str(s)) => vec![s.clone()],
                _ => vec![],
            };
    ...
    if let Some(val) = user_map.get("group_ids").or_else(|| user_map.get("user/group_ids")) {
        ...
        while let Some(g_id) = queue.pop() {
            ...
            if let Some(loc_val) = group_map.get("allowed_locations").or_else(|| group_map.get("user_group/allowed_locations")) {
            ...
            if let Some(asset_val) = group_map.get("allowed_assets").or_else(|| group_map.get("user_group/allowed_assets")) {
            ...
            if let Some(tr_val) = group_map.get("time_restrictions").or_else(|| group_map.get("user_group/time_restrictions")) {
            ...
            if let Some(parent_val) = group_map.get("parent_user_group_id").or_else(|| group_map.get("user_group/parent_user_group_id")) {
    ...
    if let Some(val) = user_map.get("time_restrictions").or_else(|| user_map.get("user/time_restrictions")) {
...
pub async fn get_principal_data<T>(
    ...
    if std::env::var("METRI_TEST_MODE").unwrap_or_default() == "1" {
        let test_tenant = req.metadata().get("test-tenant")
            .and_then(|v| v.to_str().ok())
            .unwrap_or("system")
            .to_string();
        let test_user = req.metadata().get("test-user")
            .and_then(|v| v.to_str().ok())
            .unwrap_or("usr_system_bff")
            .to_string();
        let test_roles = req.metadata().get("test-roles")
            .and_then(|v| v.to_str().ok())
...
mod tests {
    ...
    fn test_cedar_contractor_isolation() {
        ...
        {
            let principal = "Metri::User::\"usr_internal\"".parse::<EntityUid>().unwrap();
            let action = "Metri::Action::\"QueryMetrics\"".parse::<EntityUid>().unwrap();
            let resource = "Metri::MetriResource::\"res_project_a\"".parse::<EntityUid>().unwrap();
            let request = Request::new(
                Some(principal),
                Some(action),
                Some(resource),
            ...
            let result = authorizer.is_authorized(&policies, &entities, &request).unwrap();
            assert!(result, "Internal user should be allowed to access project A");
        }

        // SCENARIO 2: Contractor of Provider A accesses project A (assigned to Provider A) -> ALLOW
        {
            let principal = "Metri::User::\"usr_contractor_a\"".parse::<EntityUid>().unwrap();
            let action = "Metri::Action::\"QueryMetrics\"".parse::<EntityUid>().unwrap();
            let resource = "Metri::MetriResource::\"res_project_a\"".parse::<EntityUid>().unwrap();
            let request = Request::new(
                Some(principal),
            ...
            let result = authorizer.is_authorized(&policies, &entities, &request).unwrap();
            assert!(result, "Contractor of Provider A should be ALLOWED to access project A");
        }
        ...
        // SCENARIO 3: Contractor of Provider A accesses project B (assigned to Provider B) -> DENY
        {
            let principal = "Metri::User::\"usr_contractor_a\"".parse::<EntityUid>().unwrap();
            let action = "Metri::Action::\"QueryMetrics\"".parse::<EntityUid>().unwrap();
            let resource = "Metri::MetriResource::\"res_project_b\"".parse::<EntityUid>().unwrap();
            let request = Request::new(
                Some(principal),
                Some(action),
                Some(resource),
            ...
            let result = authorizer.is_authorized(&policies, &entities, &request).unwrap();
            assert!(!result, "Contractor of Provider A should be DENIED access to project B");
        }

        // SCENARIO 4: Suspended internal user accesses project A -> DENY
        {
            let principal = "Metri::User::\"usr_suspended_internal\"".parse::<EntityUid>().unwrap();
            let action = "Metri::Action::\"QueryMetrics\"".parse::<EntityUid>().unwrap();
            let resource = "Metri::MetriResource::\"res_project_a\"".parse::<EntityUid>().unwrap();
            let request = Request::new(
                Some(principal),
    ...
    async fn test_user_suspended() {
        let valkey = InMemorySessionStore::new();
        valkey.insert("token_suspended", Session {
        ...
        let ddb = Arc::new(crate::infrastructure::dynamodb::DynamoClient::new("metri-dynamo").await);
    ...
    async fn test_cache_hit_prevents_db_query() {
        ...
        cache.store_principal("usr_cached", principal).await.unwrap();

        let ddb = Arc::new(crate::infrastructure::dynamodb::DynamoClient::new("invalid-table").await);
    ...
    async fn test_user_group_hierarchy_and_refinement() {
        ...
        user_map.insert("user_type".to_string(), DatomValue::Str("INTERNAL".to_string()));
        user_map.insert("status".to_string(), DatomValue::Str("ACTIVE".to_string()));
        user_map.insert("role_ids".to_string(), DatomValue::Array(vec!["role_user".to_string()]));
        user_map.insert("group_ids".to_string(), DatomValue::Array(vec!["group_sub".to_string()]));

        let mut role_map = HashMap::new();
        role_map.insert("grants".to_string(), DatomValue::Str("[\"work_order\"]".to_string()));
        role_map.insert("permitted_locations".to_string(), DatomValue::Array(vec!["loc_root_role".to_string(), "loc_other_role".to_string()]));
        role_map.insert("permitted_assets".to_string(), DatomValue::Array(vec!["asset_root_role".to_string()]));

        let mut group_sub_map = HashMap::new();
        group_sub_map.insert("allowed_locations".to_string(), DatomValue::Array(vec!["loc_sub_group".to_string()]));
        group_sub_map.insert("allowed_assets".to_string(), DatomValue::Array(vec!["asset_sub_group".to_string()]));
        group_sub_map.insert("parent_user_group_id".to_string(), DatomValue::Str("group_parent".to_string()));

        let mut group_parent_map = HashMap::new();
        group_parent_map.insert("allowed_locations".to_string(), DatomValue::Array(vec!["loc_root_role".to_string(), "loc_new_group".to_string()]));
        group_parent_map.insert("allowed_assets".to_string(), DatomValue::Array(vec!["asset_root_role".to_string()]));
        group_parent_map.insert("time_restrictions".to_string(), DatomValue::Str("[{\"days_of_week\":[1,2,3,4,5],\"start_minute\":480,\"end_minute\":1080}]".to_string()));

        let user_map_clone = user_map.clone();
        // Populate global EAV Cache
        {
            let mut cache = EAV_CACHE.write().unwrap();
            cache.insert("T#tnt_01#E#usr_hierarchy_test".to_string(), CacheEntry {
                is_complete: true,
                map: user_map,
            });
            cache.insert("T#tnt_01#E#role_user".to_string(), CacheEntry {
                is_complete: true,
                map: role_map,
            });
            cache.insert("T#tnt_01#E#group_sub".to_string(), CacheEntry {
                is_complete: true,
                map: group_sub_map,
            });
            cache.insert("T#tnt_01#E#group_parent".to_string(), CacheEntry {
                is_complete: true,
        ...
        let principal = assemble_principal_graph(&eav_reader, "tnt_01", "usr_hierarchy_test", user_map_clone).await.unwrap();

        assert_eq!(principal.user_id, "usr_hierarchy_test");
        assert!(principal.roles.contains("role_user"));
        
        // Check user group attributes union:
        assert!(principal.group_allowed_locations.contains(&"loc_sub_group".to_string()));
        assert!(principal.group_allowed_locations.contains(&"loc_root_role".to_string()));
        assert!(principal.group_allowed_locations.contains(&"loc_new_group".to_string()));

        assert!(principal.group_allowed_assets.contains(&"asset_sub_group".to_string()));
        assert!(principal.group_allowed_assets.contains(&"asset_root_role".to_string()));

        assert_eq!(principal.time_restrictions.len(), 1);
        assert_eq!(principal.time_restrictions[0].days_of_week, vec![1,2,3,4,5]);

        // 3. Consolidate (performs expansion and intersection)
        let consolidated = step3_consolidate(&eav_reader, "tnt_01", principal).await.unwrap();
    ...
    fn test_tenant_domain_matrix_system_vs_normal() {
        ...
        let system_principal_ok = PrincipalData {
            ...
            roles_boundaries: vec![RoleBoundary {
                ...
                grants: vec![serde_json::json!({"domain": "tenant", "actions": ["VIEW"], "scope": "ALL"})],
                permitted_locations: vec![],
                permitted_assets: vec![],
            }],
            time_restrictions: vec![],
        ...
        assert_eq!(boundaries[0].get("query_scope").unwrap().as_str().unwrap(), "ALL");
        ...
        let system_principal_err = PrincipalData {
            ...
            roles_boundaries: vec![RoleBoundary {
                ...
                grants: vec![serde_json::json!({"domain": "work_order", "actions": ["VIEW"], "scope": "ALL"})],
                permitted_locations: vec![],
                permitted_assets: vec![],
            }],
            time_restrictions: vec![],
    ...
    fn test_cedar_native_action_validation() {
        ...
        assert!(res_create.is_ok(), "CREATE should be allowed with project:CREATE grant: {:?}", res_create);
        ...
        assert!(res_delete.is_err(), "DELETE should be denied when user only has project:CREATE grant");
    ...
    fn test_mutational_abac_hydration() {
        ...
        assert!(res_matching.is_ok(), "Contractor should be allowed to update resource assigned to their company");
        ...
        assert!(res_different.is_err(), "Contractor should be denied access to a different company's resource");
        ...
        assert!(res_none.is_err(), "Contractor should be denied access if resource has no company assigned");
    ...
    fn test_system_entities_authorization() {
        ...
        for entity in &system_entities {
            ...
            let sys_res = SystemSecurityRules::check_crud_authorization(entity, "system", "usr_sys", "mutate");
            assert!(sys_res.is_ok(), "Expected system tenant to be authorized for {}", entity);

            // Non-system tenant user should be rejected by check_crud_authorization
            let reg_res = SystemSecurityRules::check_crud_authorization(entity, "tnt_tenant1", "usr_reg", "mutate");
            assert!(reg_res.is_err(), "Expected non-system tenant to be rejected for {}", entity);
        ...
        let view_res = step4_analytical(&cedar_auth, &principal_sys_with_grant, "VIEW", &resource_domain_plugin);
        assert!(view_res.is_ok(), "System user with grant should view domain_plugin");

        // 2. System tenant user without grant -> unauthorized
        let view_no_grant_res = step4_analytical(&cedar_auth, &principal_sys_no_grant, "VIEW", &resource_domain_plugin);
        assert!(view_no_grant_res.is_err(), "System user without grant should be rejected");
