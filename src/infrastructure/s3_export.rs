...
impl IExportStorage for S3ExportStorage {
    async fn generate_presigned_url(
        ...
        if let Err(e) = wtr.write_record(&headers) {
            ...
            return Err(DomainError::new(ErrorCode::Infra002, format!("CSV Header formatting error: {e}")));
        }
        ...
        for row in rows {
            ...
            if let Err(e) = wtr.write_record(&record) {
                ...
                return Err(DomainError::new(ErrorCode::Infra002, format!("CSV Row formatting error: {e}")));
            }
        }
        ...
        let csv_bytes = match wtr.into_inner() {
            ...
            Err(e) => {
                ...
                return Err(DomainError::new(ErrorCode::Infra002, format!("CSV buffer finalization error: {e}")));
            }
        };
        ...
        info!("[S3ExportStorage] Uploading CSV to S3: s3://{}/{}", self.bucket, key);
        
        let body = aws_sdk_s3::primitives::ByteStream::from(csv_bytes);
        if let Err(e) = self.client.put_object()
            .bucket(&self.bucket)
            .key(&key)
            .body(body)
            ...
            .await 
        {
            ...
            return Err(DomainError::new(ErrorCode::Infra002, format!("S3 upload error: {e}")));
        }
            ...
            .build()
        {
            ...
            Err(e) => {
                ...
                return Err(DomainError::new(ErrorCode::Infra002, format!("S3 presigning configuration error: {e}")));
            }
        };

        let presigned_req = match self.client.get_object()
            .bucket(&self.bucket)
            .key(&key)
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                return Err(DomainError::new(ErrorCode::Infra002, format!("S3 presigned URL generation error: {e}")));
            }
        };
...
impl IExportStorage for StubExportStorage {
    async fn generate_presigned_url(
        ...
        let mock_url = format!("https://metri-mock-exports.s3.amazonaws.com/exports/mock_file_{mock_uuid}.csv");
...
fn value_to_csv_field(v: &prost_types::Value) -> String {
    ...
    match &v.kind {
        ...
        Some(Kind::StructValue(s)) => {
            serde_json::to_string(&crate::grpc::translator::struct_to_value(s.clone())).unwrap_or_default()
        }
        Some(Kind::ListValue(l)) => {
            let arr: Vec<Value> = l.values.iter()
