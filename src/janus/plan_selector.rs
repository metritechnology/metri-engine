...
pub enum EavQueryPlan {
    ...
    AvetSingleFilter { attr_name: String, value: DatomValue },
    ...
    AevtScan { entity_type: String, shard_total: Option<u8> },

    /// Reverse reference lookup → GSI-VAET
    VaetLookup { ref_entity_id: String, attr_name: Option<String> },
...
pub fn select_plan(ast_ir: &Value) -> EavQueryPlan {
    let entity_type = ast_ir.get("entity")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown")
        .to_string();
    ...
    if let Some(as_of) = ast_ir.get("as_of_tx").and_then(|v| v.as_u64()).filter(|&t| t > 0) {
    ...
    match indexed_filters.len() {
        ...
        _ => {
            ...
            EavQueryPlan::AvetIntersection { filters: indexed_filters }
        }
    }
}
...
fn json_to_datum_value(val: &Value) -> DatomValue {
    match val {
        ...
        Value::Number(n) => {
            if let Some(i) = n.as_i64() { DatomValue::Long(i) }
            else if let Some(f) = n.as_f64() { DatomValue::Double(f) }
            else { DatomValue::Null }
        }
    ...
    if let Some(filters) = &ast_ir.filters {
        for f in filters {
            if let Some(crit) = &f.criteria {
                if crit.op_ref.0 == fbs::FilterOperator::EQ.0 && crit.field.as_deref() == Some("entity/ulid") {
    ...
    if let Some(h) = &ast_ir.hierarchy {
        if let Some(node_id) = &h.current_node_id {
            if node_id != "__none__" && !node_id.is_empty() {
                let pf = h.parent_field.clone().unwrap_or_else(|| "parent_id".to_string());
