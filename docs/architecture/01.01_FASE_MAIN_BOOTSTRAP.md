...
(defn run-fail-fast!
  ...
  ;; Escanea resources/models/*.json, valida con Malli, registra en memoria.
  ;; G6 FASE 10: span OTel (SDK ya inicializado en paso [2])
  ;; NOTA D7: ig/init-key :codice/registry ejecuta seed-event-routing-rules!
  ;; y pre-calienta el sequence_registry GLOBAL usando
  ;; tenant-guard/transact-with-tenant! (no d/transact directo).
  ;; Además :codice/generator-inject propaga el tenant-guard de forma blindada al OLTPChannel.
...
(defmethod ig/init-key :grpc/server
  [_ {:keys [service-impl port enable-reflection health-manager
      ...
      :or   {port 9090
             ...
             deadline-ms 30000}}]
  (let [builder (-> (io.grpc.netty.shaded.io.grpc.netty/NettyServerBuilder/forPort port)
                    ;; ── Interceptores de transporte (orden = prioridad) ─────
                    (.intercept (metri.grpc.interceptors/otel-server-interceptor otel-sdk))
                    (.intercept (metri.grpc.interceptors/deadline-enforcement-interceptor deadline-ms))
                    (.intercept (metri.grpc.interceptors/logging-interceptor))
                    ;; ── Service + Health ─────────────────────────────────────
        ...
        builder (if enable-reflection
                  (.addService builder (io.grpc.protobuf.services/ProtoReflectionService/newInstance))
                  builder)
...
(defmethod ig/halt-key! :grpc/server
  ...
  (when-not (.awaitTermination server 30 java.util.concurrent.TimeUnit/SECONDS)
    (println "      ⚠ gRPC server did not terminate in 30s — forcing shutdown")
    (.shutdownNow server)))
```
