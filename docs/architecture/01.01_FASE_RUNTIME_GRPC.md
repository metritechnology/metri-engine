...
**Consumida por:** Clientes gRPC (`metri-auth`, `metri-data`, `Event Router`, `MCP Proxy`)
...
### VIII.1 — Modo ECS Fargate (Producción Primaria)
...
### VIII.3 — Modo Lambda (Adaptador)

Para el modo Lambda, el entry point cambia a un adaptador que traduce Lambda Function URL events al mismo pipeline IOP.
El handler usa `RequestStreamHandler` para soportar `RESPONSE_STREAM` (streaming compatible con gRPC):

```clojure
;; ns: metri.lambda.handler [NEW]
;; Adaptador Lambda Function URL → pipelines del Metri Engine.
;; Reutiliza EXACTAMENTE los mismos pipelines que el modo ECS.
;; Solo el transporte cambia — el dominio es idéntico.
(ns metri.lambda.handler
  "AWS Lambda entry point con RESPONSE_STREAM.
   Traduce Function URL events → IOP pipeline → streaming response.
   SnapStart compatible — bootstrap se ejecuta en init phase."
  (:gen-class
...
(defn -handleRequest
  "Entry point Lambda — Function URL con RESPONSE_STREAM.
   El event contiene el body JSON con la operación gRPC serializada."
  [this ^InputStream input-stream ^OutputStream output-stream context]
  (let [system    @system  ;; ← lazy init (SnapStart lo congela)
        iop       (:iop/pipeline system)
        event     (json/parse-stream (io/reader input-stream))
        ;; Determinar operación desde el path o header
        operation (get-in event [:headers :x-metri-operation] "transact")
        ;; Traducir JSON event → mapa canónico IOP
