...
> **ESTADO (2026-09-03):** **LAS SIETE FASES EJECUTADAS Y VERDES** — anexo de ejecución al final de esta sección. La causa raíz fue más profunda que la escrita aquí (metadata Iceberg borrada del lake, no solo prefijos ausentes); el remedio completo: stream `domain-fault` recreado con `WarehouseLocation` explícito + **4 tablas Iceberg materializadas por DDL de Athena** + reingesta íntegra del dato perdido con conciliación exacta. Fase 1 desplegada en producción tras cerrarse el desmonte Cedar (con un incidente de empaquetado documentado y resuelto). Fase 4: seeder idempotente verificado en vivo. Fase 5: `execute_single_query` descompuesta en 4 módulos con corpus primero. Acción opcional agendada: normalizar el `WarehouseLocation` de 3 streams (`--recreate-on-catalog-drift`); pendiente humano: dar un suscriptor al SNS de escalado.
...
| **5 — execute_single_query** | **VERDE (2026-09-03, 4 commits: corpus → sql_parse → pipeline → lake_reader).** Corpus de caracterización primero (Regla 05 heredada): 22 tests fijando el comportamiento exacto de los helpers puros ANTES de mover una línea (2 expectativas corregidas durante la escritura eran errores aritméticos del test, no del motor). Descomposición en módulos con red propia — `execute_single_query` pasó de 466 líneas a ~90 de orquestación: `mod.rs` (277: engine + orquestador), `sql_parse.rs` (724: parseo del SQL de Aegis, movimiento puro), `pipeline.rs` (697: pipeline en memoria puro — filtros, rango temporal, agregación, proyección — con 10 tests nuevos que fijan su semántica), `lake_reader.rs` (149: la única pieza con I/O). La ruta `local_s3_query_engine` se conservó para no tocar llamadores. Suite en verde en cada commit (411 al cierre). |
