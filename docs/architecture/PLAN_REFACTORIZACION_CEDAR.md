...
> **EJECUTADO (3 de septiembre de 2026):** fases 0–5 completadas en los commits `a0a099c` → `ee42555`, y los 5 pendientes del cierre resueltos después: D2 (bootstrap de políticas compartido en `engine::default_policy_cache`), 5C (decisión allow/deny y latencia por paso como eventos tracing estructurados), canal global eliminado (trait `InvalidationBus` + `BroadcastBus` inyectado desde la raíz de composición), `assemble_principal_graph` partido (ninguna función del módulo pasa de 75 líneas), `AuthenticationPolicy` sustituye al bool de bypass, y los tests de integración corren en verde contra dynamodb-local (3 audit e2e + 12 quota + 9 writer EAV). Suite unitaria: 379 tests en verde, clippy 0 en `src/cedar`. Desviación mantenida: no hay `CedarConfig` propio — la configuración única es `engine_config()` de ADR-004; la tarea de invalidación sigue naciendo en el constructor de la caché pero consume el bus inyectado.
