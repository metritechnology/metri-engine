...
def run_tests():
    logging.info("Ejecutando Consulta RAW (SELECT *) sobre asset (OLTP)...")
    
    req = metri_pb2.QueryRequest()
    req.tenant_id = "datalog-golden-tenant"
    q_name = "Raw_Select_Star_Asset_OLTP"
    ...
    with open("asset_raw_oltp.json", "w") as f:
        json.dump(results_map, f, indent=2)
    print("\nReporte JSON generado exitosamente en asset_raw_oltp.json")
