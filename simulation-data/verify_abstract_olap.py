...
def verify():
    print("==================================================")
    print("🔥 METRI ENGINE - VALIDACIÓN OLAP ABSTRACTO 🔥")
    print("==================================================")
    ...
    print("\n[3] Probando extracción abstracta desde 'data_blob'...")
    # Ejemplo: extraer reading_value de meter_reading y action_type de audit_log
    query_extract = """
    SELECT 
        _entity,
        CASE 
            WHEN _entity = 'meter_reading' THEN json_extract_scalar(data_blob, '$.reading_value')
            WHEN _entity = 'audit_log' THEN json_extract_scalar(data_blob, '$.action_type')
    ...
    if rows:
        print(f"{'_entity':15} | {'extracted_value':20} | {'data_blob (resumen)':30}")
        print("-" * 75)
        for row in rows[1:]:
            entity = row["Data"][0].get("VarCharValue", "NULL")
            val = row["Data"][1].get("VarCharValue", "NULL")
            blob = row["Data"][2].get("VarCharValue", "NULL")
            print(f"{entity:15} | {val:20} | {blob[:30]}...")
