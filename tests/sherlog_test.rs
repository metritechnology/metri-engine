...
use metri_engine::iop::sherlog::{IFaultNotifier, FaultSeverity, process_fault};
...
impl IFaultNotifier for MockFaultNotifier {
    async fn notify(&self, error_dto: &Value, severity: &FaultSeverity) -> Result<(), DomainError> {
        self.calls.lock().unwrap().push((error_dto.clone(), severity.clone()));
        Ok(())
    }
}
...
fn setup_catalog() {
    INIT.call_once(|| {
        ...
        if metri_engine::domain::error_catalog::try_global().is_none() {
            let catalog = metri_engine::domain::ErrorCatalog::load("config/errors/error_catalog.toml").unwrap();
...
async fn test_process_fault_double_action() {
    ...
    println!("DEBUG: Serialized code: {:?}", serde_json::to_value(&error.code));
    println!("DEBUG: try_global is_some: {}", metri_engine::domain::error_catalog::try_global().is_some());
    if let Some(catalog) = metri_engine::domain::error_catalog::try_global() {
        ...
        let code_str = serde_json::to_value(&error.code).ok().and_then(|v| v.as_str().map(|s| s.to_string())).unwrap();
        println!("DEBUG: Catalog entry with code_str ({}): {:?}", code_str, catalog.get(&code_str));
    }
    ...
    let data_array = fault_ctx.request.get("data").and_then(|d| d.as_array()).unwrap();
    assert_eq!(data_array.len(), 1);
    let fault_record = data_array[0].as_object().unwrap();

    assert_eq!(fault_record.get("trace_id").unwrap().as_str().unwrap(), "test_trace_12345");
    assert_eq!(fault_record.get("tenant_id").unwrap().as_str().unwrap(), "tnt_test");
    assert_eq!(fault_record.get("user_id").unwrap().as_str().unwrap(), "usr_test");
    assert_eq!(fault_record.get("error_code").unwrap().as_str().unwrap(), "JNS_001");
    assert_eq!(fault_record.get("severity").unwrap().as_str().unwrap(), "ERROR");
    assert_eq!(fault_record.get("stage").unwrap().as_str().unwrap(), "janus");
    assert_eq!(fault_record.get("component").unwrap().as_str().unwrap(), "metri-engine");
    assert_eq!(fault_record.get("entity_type").unwrap().as_str().unwrap(), "asset");
    assert_eq!(fault_record.get("retryable").unwrap().as_bool().unwrap(), false);
    assert_eq!(fault_record.get("occurred_at").unwrap().as_i64().unwrap(), 1718221000i64);
    assert_eq!(fault_record.get("context").unwrap()["field"].as_str().unwrap(), "test_field");
}
