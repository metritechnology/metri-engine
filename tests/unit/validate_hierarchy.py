...
def test():
    ...
    if roots:
        site_node = next((r for r in roots if r.get("type") == "SITE" and r.get("has_children")), roots[0])
        site_id = site_node.get("id")
        print(f"\n=== Test 3: Children de site_id={site_id} ({site_node.get('name')}) ===")
        ...
        if not children:
            ...
            if any(c.get("type") == "BUILDING" for c in children):
                print("  PASS: Al menos un BUILDING presente")
            else:
                errors.append("FAIL: ningún BUILDING en hijos")

        # ── Test 4: Grandchildren ──────────────────────────────────────────
        if children:
            bldg_node = next((c for c in children if c.get("type") == "BUILDING" and c.get("has_children")), children[0])
            bldg_id = bldg_node.get("id")
            print(f"\n=== Test 4: Floors de building={bldg_id} ({bldg_node.get('name')}) ===")
